"""
Universal OpenVINO Dependency Parser by bezoku

This is early beta code - expect issues. The runner has been tested using Terminal on Intel CPU, but should work on any openVINO supported architecture.

For support email ian.gilmour@bezoku.ai with error message after checking you have loaded the following:
torch>=2.0.0
openvino>=2023.0.0
numpy>=1.24.0

Supports multi-task inference: UPOS, XPOS, DEPREL, FEATS, HEADS
Locate the following files in each language directory before running -> model.pt, vocab.pkl, and metadata.json
"""
