...
# Grammar checking imports
try:
    from grammar_checker import GrammarChecker, print_grammar_report
...
def interactive_mode(compiled_model, vocab: Dict, metadata: Dict, input_layer, output_layers, grammar_checker=None):
    ...
    show_conllu = False
    check_grammar = grammar_checker is not None
    ...
    while True:
        try:
            ...
            if user_input.lower() in ['quit', 'exit', 'q']:
            ...
            elif user_input.lower() == 'grammar' and grammar_checker:
                check_grammar = not check_grammar
                print(f"✓ Grammar checking: {'ON' if check_grammar else 'OFF'}")
                ...
                # Grammar checking
                if check_grammar and grammar_checker:
                    errors = grammar_checker.validate(results)
                    print_grammar_report(errors, results)
...
def main():
    ...
    if GRAMMAR_CHECKER_AVAILABLE:
        print("✓ Grammar checking module available")
    ...
    # Step 4.5: Initialize grammar checker if available
    grammar_checker = None
    if GRAMMAR_CHECKER_AVAILABLE:
        model_dir = os.path.dirname(model_path)
        morph_table_path = os.path.join(model_dir, 'morphology_table.pkl')
        
        if os.path.exists(morph_table_path):
            print(f"\n✓ Loading morphology table for grammar checking...")
            morph_table = GrammarChecker.load_morphology_table(morph_table_path)
            grammar_checker = GrammarChecker(morph_table)
            print(f"✓ Grammar checking enabled")
        else:
            print(f"\n⚠️  No morphology_table.pkl found - grammar checking will run without suggestions")
            grammar_checker = GrammarChecker()
    
    # Step 5: Interactive mode
    interactive_mode(compiled_model, vocab, metadata, input_layer, output_layers, grammar_checker)
